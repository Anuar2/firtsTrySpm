...
class EditAdDocumentInfoCell: UICollectionViewCell {
    
    var documentInfo: DocumentViewModel? {
        didSet {
            reloadInputViews()
            mainTableView.reloadData()
            onDocumentChanged?(documentInfo)
        }
    }
    
    let sections: [EditAdDocumentInfoCellSection] = [.init(section: .main, rows: [.documentPicker, .docNumber, .docName, .docType, .signType, .signatories, .docDescription, .calendar])]
...
//            }
    ...
    var onDocumentChanged: ((DocumentViewModel?) -> ())?
    
    var onInitiatorsDidTap: (() -> ())?
    
    var onStartDateDidTap: (() -> ())?
...
extension EditAdDocumentInfoCell: UITableViewDataSource {
...
//                cell.textFieldText = text
        ...
        case .signatories:
            ...
            cell.onInitiatorsDidTap = {
                self.onInitiatorsDidTap?()
            }
            cell.signatureTapped = {
                self.onInitiatorsDidTap?()
            }
            return cell
        case .calendar:
...
//                self.presentStartDateCalendarView()
                self.onStartDateDidTap?()
            }
            cell.onEndDateButtonDidTap = {
                self.onEndDateDidTap?()
            }
            cell.configure(starteDateTitle: documentInfo?.startDate ?? "", endDateTitle: documentInfo?.endDate ?? "")
            return cell
        case .documentPicker:
            let cell: UploadDocumentViewCell = tableView.dequeueReusableCell(for: indexPath)
            if let url = documentInfo?.documentLink {
                cell.selectedFileURL = URL(string: url)
            }
            cell.onPickButtonDidTap = {
                self.onPickButtonDidTap?()
...
extension EditAdDocumentInfoCell: UITableViewDelegate {
...
extension EditAdDocumentInfoCell: DocTypeViewControllerProtocol {
...
extension EditAdDocumentInfoCell: InitiatorViewControllerProtocol {
...
extension EditAdDocumentInfoCell {
    struct EditAdDocumentInfoCellSection {
        enum Section {
            case main
        }
