//
//  ExpireDateViewController.swift
//
//
...
protocol ExpireDateViewControllerDelegate: AnyObject {
    func didSelectExpireDateRange(dateFrom: String, dateTo: String)
}

class ExpireDateViewController: UIViewController, ExpireDateViewInput {
    
    var output: ExpireDateViewOutput?
    var delegate: ExpireDateViewControllerDelegate?
    ...
    private lazy var navigationView: DFFloatingPanelNavBar = {
        ...
        view.navigationTitle = "Expiry date"
        return view
    }()
    ...
    @objc private func applyButtonDidTap() {
        if let dateFrom = dateFromDateLabel.text, let dateTo = dateToDateLabel.text {
            delegate?.didSelectExpireDateRange(dateFrom: dateFrom, dateTo: dateTo)
