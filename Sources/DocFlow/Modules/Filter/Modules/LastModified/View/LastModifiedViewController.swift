//
//  LastModifiedViewController.swift
//
//
...
protocol LastModifiedDateViewControllerDelegate: AnyObject {
    func didSelectLastModifiedDateRange(dateFrom: String, dateTo: String)
}

class LastModifiedViewController: UIViewController, LastModifiedViewInput {
    
    var output: LastModifiedViewOutput?
    var delegate: LastModifiedDateViewControllerDelegate?
    ...
    private lazy var navigationView: DFFloatingPanelNavBar = {
        ...
        view.navigationTitle = "Last modified"
        return view
    }()
    ...
    @objc private func applyButtonDidTap() {
        if let dateFrom = dateFromDateLabel.text, let dateTo = dateToDateLabel.text {
            delegate?.didSelectLastModifiedDateRange(dateFrom: dateFrom, dateTo: dateTo)
