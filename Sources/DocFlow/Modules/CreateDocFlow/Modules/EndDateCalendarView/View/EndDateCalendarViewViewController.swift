//
//  EndDateCalendarViewViewController.swift
//
//
...
protocol EndDateCalendarViewDelegate {
    func didEndPickDate(_ dateString: String)
}

class EndDateCalendarViewViewController: UIViewController, EndDateCalendarViewViewInput {
    
    var output: EndDateCalendarViewViewOutput?
    
    var delegate: EndDateCalendarViewDelegate?
    ...
    @objc private func applyButtonDidTap() {
        if let selectedDay = selectedDay {
            delegate?.didEndPickDate(convertDateToString(day: selectedDay))
            output?.dissmiss()
        }
    }
